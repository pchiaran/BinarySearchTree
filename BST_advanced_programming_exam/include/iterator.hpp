...
public:
	...
	//if the current node has right child, move to it because its key is greater than the current one
	  ...
	  BST_iterator operator++(int) noexcept { 	//CONTROLLA
		    BST_iterator tmp{current};
		    ++(*this);
		    return tmp; }
	  
	  //define equality between iterators if they point to the same node NOTA friend?
	  friend bool operator==(const BST_iterator& a, const BST_iterator& b) { return a.current == b.current; }
	  friend bool operator!=(const BST_iterator& a, const BST_iterator& b) { return !(a == b); }

	  // set bst class as a friend class to access iterator's members in it  CONTROLLA
	  template<typename K, typename V, typename cmp>
