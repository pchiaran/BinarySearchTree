...
//template to define a templated node class. Each node has a value of type T and three pointers, the 'right' and 'left' pointers to children nodes and the 'up' one to the parent node
	...
	class node{
		...
		node() noexcept : value{}, left{nullptr}, right{nullptr}, upper{nullptr} { std::cout << "Node default constructor" << std::endl; } 
		
    		//custom constructor to initialize a node using an lvalue. This copies an exixsting node
		node(const T& v, node<T>* up) noexcept : value{v}, left{nullptr}, right{nullptr}, upper{up} { std::cout << "Node custom constructor" << std::endl; }

		//custom constructor to initialize a node using an rvalue
		node(T&& v, node<T>* up) noexcept : value{v}, left{nullptr}, right{nullptr}, upper{up} { std::cout << "Node move constructor" << std::endl;  }
